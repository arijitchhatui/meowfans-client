...
export const SearchTerm: React.FC<Props> = ({ label }) => {
  const { vaults, loading, handleLoadMore, hasMore } = useVaults({ searchTerm: label });

  return (
    <PageWrapper>
      <h2 className="text-xl font-semibold m-3">Explore Vaults</h2>
      <InfiniteScrollWrapper dataLength={vaults.length} hasMore={hasMore} loading={loading} onLoadMore={handleLoadMore}>
