...
export const CreatorProfile: React.FC<Props> = ({ creatorAssets }) => {
  const searchParams = useSearchParams();
  const router = useRouter();
  const [currentTab, setCurrentTab] = useState<TabProps>((searchParams.get('tab') as TabProps) || 'gallery');
  const [assets, setAssets] = useState<CreatorAssetsEntity[]>(creatorAssets as CreatorAssetsEntity[]);
  ...
  return (
    <main className="p-6">
      <div className="flex items-center gap-6 mb-10">
        ...
        />
        <div>
          <h1 className="text-xs md:text-xl">@{assets[0].creatorProfile.user.username}</h1>
